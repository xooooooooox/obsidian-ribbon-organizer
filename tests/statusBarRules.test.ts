...
describe("applyStatusBarRules", () => {
  ...
  it("captures a {name} placeholder and carries it into the replacement", () => {
    ...
    expect(applyStatusBarRules("Successfully synced 2 hours ago", rules)).toEqual({ text: "✓ 2 hours ago", icon: null });
    expect(applyStatusBarRules("Successfully synced just now", rules)).toEqual({ text: "✓ just now", icon: null });
  });

  it("supports multiple placeholders", () => {
    const rules = [{ find: "{w} words{c} characters", replace: "{w}w {c}c" }];
    expect(applyStatusBarRules("22 words39 characters", rules)).toEqual({ text: "22w 39c", icon: null });
  });
  ...
  it("first matching rule wins", () => {
    ...
    expect(applyStatusBarRules("Syncing...", rules)).toEqual({ text: "first", icon: null });
  });

  it("returns the text unchanged when no rule matches (fail-open)", () => {
    expect(applyStatusBarRules("Never Synced", [{ find: "Syncing...", replace: "⟳" }])).toEqual({ text: "Never Synced", icon: null });
  });

  it("requires a full match, not a substring", () => {
    expect(applyStatusBarRules("prefix Syncing... suffix", [{ find: "Syncing...", replace: "⟳" }])).toEqual({ text: "prefix Syncing... suffix", icon: null });
  });

  it("treats regex specials in literals literally", () => {
    expect(applyStatusBarRules("(sync) 50% [done]", [{ find: "(sync) {p} [done]", replace: "{p}" }])).toEqual({ text: "50%", icon: null });
  });

  it("never matches malformed or empty templates (fail-open)", () => {
    expect(applyStatusBarRules("abc", [{ find: "a{b", replace: "x" }])).toEqual({ text: "abc", icon: null }); // unbalanced
    expect(applyStatusBarRules("abc", [{ find: "{x} and {x}", replace: "y" }])).toEqual({ text: "abc", icon: null }); // duplicate name
    expect(applyStatusBarRules("abc", [{ find: "{}", replace: "y" }])).toEqual({ text: "abc", icon: null }); // empty name
    expect(applyStatusBarRules("", [{ find: "", replace: "y" }])).toEqual({ text: "", icon: null }); // empty find never matches
  });

  it("never re-substitutes text a capture inserted", () => {
    const rules = [{ find: "{a} and {b}", replace: "{a} - {b}" }];
    expect(applyStatusBarRules("{b} and world", rules)).toEqual({ text: "{b} - world", icon: null });
  });

  it("leaves unknown placeholders in the replacement as literal text", () => {
    expect(applyStatusBarRules("hi", [{ find: "hi", replace: "{other}" }])).toEqual({ text: "{other}", icon: null });
  });
