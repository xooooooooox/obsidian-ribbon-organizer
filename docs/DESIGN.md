...
- Pill bottom baseline is per-layout: phones clear the bottom navbar (`--mobile-toolbar-height` + gap, no safe-area term — the navbar box already covers the home indicator); tablets have no bottom navbar and deliberately ignore the safe-area envelope too — the home indicator is horizontally centered while the bar sits in the right corner (no overlap), and a hardware keyboard hides the indicator entirely, so reserving the 20px band read as a dead gap. A flat 10px keeps the pill off the very edge, where the swipe-up and Quick Note corner gestures start. `--ribbon-organizer-sb-gap` means the same thing in both: the gap above that baseline (navbar top on phones at 32px, window bottom on tablets at 10px).
- Tablet style is a two-option dropdown, not a second toggle: `Floating pill` (default) keeps the pill; `Docked in the corner` (body class `ribbon-organizer-sb-docked`, only meaningful with `.is-tablet`) squares the same bar flush against both edges — corner radius only on the top-left, no shadow, same height as the pill, exactly the desktop status bar's shape. Phones ignore the setting.
