...
- **`ui/GroupsSection.ts`** — the Ribbon groups tab: a single column mirroring the ribbon's final order (group headers mark where dividers render), with in-place filter, HTML5 drag-and-drop (items within/across groups, whole groups onto headers), a ⋮ "Move to group" menu, and inline rename. One instance lives on the SettingTab so filter text survives re-renders; `persist()` = save → `applyGrouping()` → re-render own container (outer scroll position holds).
