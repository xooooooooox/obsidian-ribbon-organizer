...
10. **`statusBarOrder: []` means "never touched"**: the apply pass is a strict no-op and the bar stays native. Orders are inline flex `order` values; unload clears them all.
