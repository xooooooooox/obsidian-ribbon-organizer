...
export function defaultMenus(): QuickMenu[] {
  return [{ id: crypto.randomUUID(), name: "Quick menu", icon: "ribbon-organizer", entries: [] }];
}
...
export function normalizeMenus(menusRaw: unknown, legacyQuickCommands: unknown): QuickMenu[] {
  ...
  if (Array.isArray(legacyQuickCommands)) {
    return [{ id: crypto.randomUUID(), name: "Quick menu", icon: "ribbon-organizer", entries: normalizeEntries(legacyQuickCommands) }];
  }
  return defaultMenus();
}
