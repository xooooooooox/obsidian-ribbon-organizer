...
- **Ribbon groups** (desktop): order the ribbon icons into named groups from **Settings → Ribbon Organizer** — drag icons between groups, drag groups to reorder. A thin divider line renders between adjacent non-empty groups. Groups start collapsed (header shows a member count) — click a header to expand; filtering reveals matches inside collapsed groups. Icons you haven't assigned fall into the ungrouped bucket, so newly installed plugins land in a predictable spot. Visibility stays native: hide/unhide icons via Obsidian's right-click as usual.
