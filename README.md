...
- **Ribbon groups** (desktop): order the ribbon icons into named groups from **Settings → Ribbon Organizer** — drag icons between groups, drag groups to reorder. A thin divider line renders between adjacent non-empty groups. Groups start collapsed (header shows a member count, or a visible/total pill when some members are hidden) — click a header to expand; filtering reveals matches inside collapsed groups. Icons you haven't assigned fall into the ungrouped bucket, so newly installed plugins land in a predictable spot.
