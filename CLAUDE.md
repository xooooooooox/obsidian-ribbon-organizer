...
- Errors must carry context (group id, item id, command id). No silent fallback — the sanctioned incompatibility paths are the `ribbonInternals()`/statusBar null → Notice + session latch, the known-arranger conflict standdown (recoverable, no override — `RIBBON_ARRANGER_PLUGINS` only admits plugins that provably rearrange the ribbon whenever enabled), and the observer burst breaker (session latch); nothing else degrades silently.
